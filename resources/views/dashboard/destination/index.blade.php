        ...
        <table id="invoice-list" class="table table-hover bg-white border rounded-lg">
          ...
          <tbody>
            ...
            <tr role="row">
              ...
              <td class="align-middle"><span class="inv-number">#{{ $destination->id }}</span>
              </td>
              <td class="align-middle">
                <div class="d-flex align-items-center">
                  ...
                  <p class="align-self-center mb-0 user-name">{{ $destination->state }}</p>
                </div>
              </td>

              <td class="align-middle"><span class="text-success pr-1"><i class="fal fa-calendar"></i></span>{{ \Carbon\Carbon::parse($destination->created_at)->format('d M Y') }}</td>
              ...
              <td class="align-middle">
                <form method="post" action="{{ route('destinations.destroy', $destination->id) }}">
                  @method('delete')
                  @csrf
                  <button type="submit" onclick="return confirm('Are you sure you want to delete this destination?')" class="btn btn-sm btn-default" style="border-top-left-radius: 0; border-bottom-left-radius: 0"><i class="fa fa-trash"></i></button>
              </form>
